...
//java -> db 접속해서 insert text insert into id, pw, name을 적어준다..
            int count = pstmt.executeUpdate();//insert data 1개
            Assertions.assertEquals(1, count); //1줄짜리 실행
        }
    }
    ...
    public void selectUserById() throws SQLException {
        ...
        try(PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                if(rs.next()) {//찾아오는 것이 여러줄 - while 문 , 찾아오는 것이 한줄 if문
                    System.out.println(rs.getString("name"));
                } else {
